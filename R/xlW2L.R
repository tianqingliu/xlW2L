...
#' @param filePath the Excel file with extension .xlsx, .xls, xlsm.
...
#' @return xlW2L(..)$x1T or xlW2L(..)[1] -- the original Excel table;  xlW2L(..)wideF or xlW2L(..)[2] -- the wide form after cleaning up;  xlW2L(..)longF or xlW2L(..)[3] -- the long data form.
#' @keywords Excel
#' @export
#' @examples
...
xlW2L <- function(filePath, shtNum = 1, celIStart, cellEnd, x1cell, x1Name = "", yName = "") {
  ...
  if (xsnum > 0) {
    ...
    for (i in 1:xsnum) {
      ...
      # i])) { : argument is of length zero'
      ## NA for header is treated as charaters in is.na(names(wideF)[i])
    }
  ...
  if (xsnum - ncd < 1) {
    ...
    longF <- reshape(wideD, 
      ...
      varying = vari, 
      direction = "long", 
      new.row.names = 1:(nrow(wideD) * length(vari)), 
      times = names(wideD)[vari])  #to preserve original 'numeric' column names in long column
    # timevar=names(wideD)[xsnum+1-ncd], idvar=idvari,
    # drop=names(wideD)[xsnum+1-ncd], #drop colX1 column, ncd -- adjust for col deleted 
    ##new.row.names = 1:(nrow(wideD)*length(vari)), #1: new long row numbers
    ...
    longF <- reshape(wideD, 
      timevar = names(wideD)[xsnum + 1 - ncd], 
      v.names = yName, 
      varying = vari, 
      idvar = idvari, 
      drop = names(wideD)[xsnum + 1 - ncd], 
      direction = "long", 
      new.row.names = 1:(nrow(wideD) * length(vari)), 
      times = names(wideD)[vari])  #to preserve original 'numeric' column names in long column
    ## drop=names(wideD)[xsnum+1-ncd], #drop colX1 column, ncd -- adjust for col deleted 
